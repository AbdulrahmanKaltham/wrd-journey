...
import { Mail, Lock, Sparkles, LogIn, AlertCircle, CheckCircle2, UserPlus, KeyRound, ArrowRight, Shield } from 'lucide-react';
...
export const LoginPage: React.FC<LoginPageProps> = ({ onGoToSignup }) => {
  ...
  const handlePromoteAdmin = async (e: React.FormEvent) => {
    ...
    if (!adminTargetEmail.trim()) {
      setAdminPromoteError('يرجى كتابة البريد الإلكتروني للمدير المراد ترقيته.');
      return;
    }
    ...
    try {
      const res = await fetch('/api/admin/promote-admin', {
        ...
        body: JSON.stringify({ email: adminTargetEmail.trim() }),
      });
      const data = await res.json();
      if (!res.ok || !data.success) {
        setAdminPromoteError(data.error || 'تعذر ترقية الحساب إلى مدير. تأكد من أن البريد مسجل مسبقاً.');
      } else {
        setAdminPromoteMsg(data.message || 'تمت ترقية الحساب إلى مدير بنجاح! يمكنك الآن تسجيل الدخول به.');
        setEmail(adminTargetEmail.trim());
      }
    } catch (err: any) {
      setAdminPromoteError(err.message || 'حدث خطأ في الاتصال بالخادم.');
    } finally {
  ...
  return (
    <div className="min-h-screen bg-[#F8F9F5] flex items-center justify-center p-4 font-arabic antialiased text-right">
      <div className="bg-white border-2 border-[#E0E0E0] rounded-3xl p-6 max-w-sm w-full shadow-2xl space-y-4">
        ...
        <div className="text-center pt-2 border-t border-gray-100 flex flex-col gap-2">
          <button
            ...
            className="text-xs text-[#006304] font-bold hover:underline inline-flex items-center justify-center gap-1 cursor-pointer"
          >
          ...
          <button
            ...
            className="text-[11px] text-gray-400 hover:text-slate-800 font-bold inline-flex items-center justify-center gap-1 cursor-pointer transition-colors"
          >
            <Shield className="w-3 h-3 text-[#F9BF3B]" />
      ...
      {showAdminModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/70 backdrop-blur-xs flex items-center justify-center p-4 font-arabic">
          <div className="bg-white rounded-3xl p-6 max-w-sm w-full shadow-2xl border-2 border-slate-900 space-y-4 text-right">
            <div className="flex items-center justify-between">
              <h3 className="font-heading font-black text-base text-slate-900 flex items-center gap-2">
                <Shield className="w-5 h-5 text-[#006304]" />
                <span>تهيئة حساب المدير (Admin)</span>
              </h3>
              <button
            ...
            <p className="text-xs text-gray-600 leading-relaxed">
              إذا كان لديك حساب مسجل في التطبيق وتريد ترقيته ليصبح <strong>مدير النظام</strong> ذو الصلاحيات الإدارية الكاملة، اكتب بريدك الإلكتروني هنا:
            </p>
            <form onSubmit={handlePromoteAdmin} className="space-y-3">
              <div>
                <input
                  ...
                  className="w-full pl-3 pr-3 py-2.5 rounded-xl border-2 border-gray-200 text-xs font-bold text-slate-900 focus:border-[#006304] focus:outline-hidden text-left dir-ltr"
                  required
                />
              </div>
              ...
              <div className="flex gap-2 pt-1">
                <button
                  type="button"
                  onClick={() => setShowAdminModal(false)}
                  className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold py-2.5 rounded-xl text-xs cursor-pointer"
                >
                  إغلاق
                </button>
                <button
                  ...
                  className="flex-1 bg-slate-900 hover:bg-slate-800 text-white font-bold py-2.5 rounded-xl text-xs cursor-pointer"
                >
                  {adminPromoteLoading ? 'جاري الترقية...' : 'ترقية لمدير'}
                </button>
              </div>
            </form>
          </div>
